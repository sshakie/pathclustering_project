...
from data.sql.__all_models import OrderRelations
...
class OrdersResource(Resource):
    def get(self, order_id):
        if current_user.is_authenticated:
            ...
            project_id = session.query(OrderRelations).filter(OrderRelations.order_id == order_id).first()
...
class OrdersListResource(Resource):
    def get(self):
        if current_user.is_authenticated:
            ...
            order_ids = [session.query(OrderRelations).filter(OrderRelations.project_id == i).all() for i in projects]
            orders = [session.get(Order, i) for i in order_ids]
            session.close()
    ...
    def post(self):
        if current_user.is_authenticated:
            if current_user.status == 'admin':
                ...
                project = session.query(Project).filter(Project.id == project_id).first()
                if project.admin_id == current_user.id:
                    project.orders.append(order)
                    session.commit()
                    session.close()
                    return jsonify({'success': 'created!'})
                session.close()
                abort(403, message=f"This project is not exists")
